<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.gangweitupian" label="岗位图片" prop="gangweitupian">
          <file-upload
          tip="点击上传岗位图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.gangweitupian" label="岗位图片" prop="gangweitupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.gangweitupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="内容详情" prop="neirongxiangqing">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.neirongxiangqing" label="内容详情" prop="neirongxiangqing">
                    <span>{{ruleForm.neirongxiangqing}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shixineirong/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shixineirongCrossAddOrUpdateFlag = false;
						 this.parent.search();
